...
const parseActivity = (data, activityData) => {
  return activityData !== undefined ? `"${data.spotify.song}"  <a class="spotify-time">${activityData.current} ─ ${activityData.end}</a>` : 'Not listening to anything right now.';
};
...
const reloadUser = async (data) => {
  ...
  <a class="discord" href="https://discord.com/users/935847646508228608/" target="_blank">Discord</a>`);
  ...
  ${data.spotify.artist.replace(/;/g, ',')} in Spotify.`);
    activityTimer(data);
  } else {
...
function showGithubRepos() {
  const getGitHubData = async () => {
    const response = await fetch(properties.github_api.baseURL + '/users/anjpp/repos');
...
function loadIcons() {
  ...
  $('location > a').text('Istanbul, Turkey');
  ...
  const socials = [
    {
      ...
      href: 'https://instagram.com/totallyytot',
      tippy: 'Instagram'
    },
    {
    ...
    },
    {
      ...
      href: `https://discord.com/users/935847646508228608/`,
      tippy: 'Discord'
    },
    {
